...
def create_verilog_objects_from_json(json_file):
    """
    ...
    Returns:
    list: List of VerilogModule objects.
    """
    ...
    for module_data in module_data_list:
        verilog_module = VerilogModule(
            ...
            unique_id=generate_unique_id()  # Function to generate unique ID
        )
        ...
        verilog_modules.append(verilog_module)
...
def create_module_hierarchy(modules):
    ...
    Parameters:
    - modules (list): List of VerilogModule objects.
    ...
    for module in modules:
        # Initialize the dictionary of instances for the current module
        instance_dict = {}

        for instance in module.instances:
            if instance.cell_type == 'hierarchical':
                # Append the instance name to the list corresponding to ref_name
                if instance.ref_name in instance_dict:
                    instance_dict[instance.ref_name].append(instance.name)
                else:
                    instance_dict[instance.ref_name] = [instance.name]

        # Add the current module to the hierarchy with its instances dictionary
        hierarchy[module.name] = instance_dict

    return dict(hierarchy)

def create_module_hierarchy_json(hierarchy):
    # Dump the hierarchy to a JSON file
    output_file_path = 'module_hierarchy.json'
    with open(output_file_path, 'w') as output_file:
        json.dump(module_hierarchy, output_file, indent=2)

    print(f"Hierarchical structure saved to {output_file_path}")    

def visualize_hierarchy(module_hierarchy, indent=0):
    """
    Visualize the hierarchical representation of Verilog modules.

    Parameters:
    - module_hierarchy (dict): Hierarchical representation of Verilog modules.
    - indent (int): The current indentation level.
    """
    for module, instances in module_hierarchy.items():
        print(' ' * indent + module + ' : {')
        for ref_name, names in instances.items():
            print(' ' * (indent + 5) + f'{ref_name} : {names}')
        print(' ' * indent + '}')
...
if __name__ == "__main__":
    ...
    verilog_modules = create_verilog_objects_from_json(file_path)

    # Create a hierarchical representation of Verilog modules
    module_hierarchy = create_module_hierarchy(verilog_modules)

    # Visualize the hierarchical representation
    visualize_hierarchy(module_hierarchy)

    # Generate a json file for the module hierarchy
    create_module_hierarchy_json(module_hierarchy)
